...
const { resolveDeliverySlot } = require("../utils/deliverySlotResolver");

//1. checkout controller after the updated less fields in cart model
const checkOut = async (req, res) => {
  ...
  const slot = await DeliverySlot.findOne({
    where: { id: deliverySlotId, isActive: true }
  });
  ...
  //Check slot has not expired since user loaded the page
  if(currentTime > slot.cutoffTime && slot.offsetDays === 0) {
    return res.status(400).json({
      status: 'fail',
      message: 'This delivery slot is no longer available. Please select another.'
    });
  }
  ...
  const estimatedDelivery = `${dateStr}, ${slot.windowLabel}`
